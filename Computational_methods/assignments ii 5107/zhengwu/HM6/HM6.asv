...
x_e(:,1) = mvnrnd(ones(d,1),W);

for i=2:N
    
    %generate the prediction
    tmp =  mvnrnd(ones(d,1),W);
    
    x_tilte(i,:) = A*x_e(i-1)+tmp;
    
    %compute the weights
    tmpv = 0.1*randn(1,100);
    %w(i,:) = x_tilte(i,:).^2 + tmpv;
    
    %normalize the weight;
    w(i,:) = normpdf(y(i),x_tilte(i,:).^2,0.1);
    w_n(i,:) = w(i,:)/sum(w(i,:));
    W_n(i,:) = cumsum(w_n(i,:));
...
%subplot(2,1,1)
plot(1:200,x,'r-')
hold on, plot(1:200,theta_hat,'-.');
%hold on, plot(1:200,x,'r-.');

%compute the R^2

X_r = theta_hat(11:200)';
M_X = mean(X_r);
R_X= 1-sum((x(11:200)-X_r).^2)/sum((x(11:200)-M_X).^2);
